...
from models.manager_nyx import *
...
class SingleHeadCrossAttention(nn.Module):
    def __init__(self, feature_dim, feature_dim_1d):
        ...
        self.value_adapter = nn.Sequential(
            nn.Linear(feature_dim+feature_dim_1d, 128),
            nn.ReLU(),
    ...
    def forward(self, query, keys, values, top_k=16, chunk_size=128, param_feats=None):
        ...
        for start in range(0, keys.shape[0], chunk_size):
            ...
            partial_scores = torch.einsum('bid,jd->bij', Q, K_chunk) / scale  # (batch_size, 1, chunk_size)
...
class KVMemoryModel(nn.Module):
    def __init__(self, feat_shapes, num_entries=1024, key_dim=3, feature_dim_3d=64, feature_dim_1d=64, 
        ...
        self.cross_attn_experts = nn.ModuleList([
            SingleHeadCrossAttention(self.feature_dim_3d, feature_dim_1d) for _ in range(self.n_experts)
        ])
        ...
        self.memory_values_list = torch.nn.ParameterList([
            nn.Parameter(torch.randn(num_entries, self.feature_dim_3d), requires_grad=True) for _ in range(self.n_experts)
        ])
        
        # Values initialization:
        for j in range(self.n_experts):
            torch.nn.init.xavier_normal_(self.cross_attn_experts[j].W_q.weight)
            torch.nn.init.xavier_normal_(self.cross_attn_experts[j].W_k.weight)
            torch.nn.init.xavier_normal_(self.cross_attn_experts[j].W_v.weight)
