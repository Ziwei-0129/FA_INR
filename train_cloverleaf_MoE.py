...
def parse_args():
    ...
    parser.add_argument("--base_model_config", type=str, required=False,
                        default='configs/cloverleaf/default_models.yaml',
...
def main(args):
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        total_loss = 0
        e_rndidx = torch.randint(0, len(training_dicts), (nEnsembleGroups_per_epoch * nEnsemble,))
        for egidx in range(nEnsembleGroups_per_epoch):
            ...
            for eidx in range(nEnsemble):
                ...
                curr_scalar_field = ReadMPASOScalar(training_dicts[e_rndidx[egidx*nEnsemble + eidx]]['file_src']).reshape(-1)
                curr_scalar_field = (curr_scalar_field-dmin) / (dmax-dmin)
                curr_scalar_field = torch.from_numpy(curr_scalar_field)
                curr_params = training_dicts[e_rndidx[egidx*nEnsemble + eidx]]['params'].reshape(1,6)
            ...
            for field_idx in range(num_sf_batches):
                ...
                for eidx in range(nEnsemble):
                    #####
                    rnd_idx = torch.randint(0, scalar_fields[eidx].shape[0], (batch_size_per_field,))
                    ######
                ...
                if args.base_model is None: # if using MoE
                    ...
                    model_output = fwd_fn(
                        ...
                        params_batch.view(nEnsemble, -1, 6),
                        batch_e_indices
                    )
        ...
        if (epoch+1) % args.log_every == 0:
            ...
            plt.savefig(os.path.join(args.dir_outputs, 'clover3d_fg_inr_loss_' + network_str + '.jpg'))
            plt.clf()
